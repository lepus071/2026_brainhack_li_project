#!/usr/bin/env python3
# ==============================================================================
# run_between_within_decomposition.py  (version_2: Smith et al. 2006 dual-rate, 2-var)
#
# Follow-up to run_marginal_r2_analysis.py.
#
# The raw marginal R^2 of x_f/x_s may be driven by BETWEEN-SUBJECT (trait-level)
# differences in overall x_f/x_s magnitude, which the Riemannian per-subject
# recentering (Phase 3) and per-subject behavioral fitting (Phase 0B) are
# specifically designed to make comparable across subjects.
#
# This script splits each variable (X = x_f/x_s, Y = brain ROI features)
...
def load_roi_labels(n_rois):
    """Load the feature-column -> ROI-name mapping produced by fix_roi_labels.py
    (Phase 3 sanity check). This is the authoritative alignment for the 481-dim
    feature vectors, as opposed to the raw Combined_Atlas_Labels.csv ordering."""
    if not os.path.exists(ROI_LABELS):
        return [f"ROI_{i}" for i in range(n_rois)]
    df = pd.read_csv(ROI_LABELS).sort_values('feature_col')
    labels = df['label'].tolist()
    if len(labels) != n_rois:
        print(f"  ! Warning: {len(labels)} labels in {ROI_LABELS} vs {n_rois} ROIs - "
...
def main():
    ...
    print("Loading and aligning data (same as Phase 4, v2)...")
    ...
    print("\nDone. Saved Between_{xf,xs}_r.npy and Within_{xf,xs}_R2.npy to ml_results_v2/.")
