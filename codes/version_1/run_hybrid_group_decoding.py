#!/usr/bin/env python3
# ==============================================================================
# run_hybrid_group_decoding.py
#
# Hybrid Dynamic Neural Decoding Pipeline - Phase 4 (VPA version):
# Joint Encoding & Variance Partitioning
#
# This script executes:
# 1. Read Riemannian projected feature matrix (N_windows, 464) and behavioral data.
# 2. Use "behavioral parameters (e, x_f, x_s)" as X to predict "brain features (464 dimensions)" as Y.
# 3. Train 7 combined Ridge models via Leave-One-Subject-Out (LOSO).
# 4. Calculate 7 variance partitions (3 Unique, 3 Pairwise Shared, 1 Global Shared).
# 5. Output R^2 numpy arrays containing 464 nodes for subsequent inverse mapping to 3D NIfTI brain maps.
# ==============================================================================
...
AFNI_DIR     = f'{PROJ_DIR}/data/derivatives/afni'
...
def load_and_align_data(subjects):
    """Read feature and behavioral data, and perform physiological HRF convolution and window alignment"""
    ...
    for subj in subjects:
        ...
        if subj_behav.empty: continue
        
        onsets = 16.5 + (subj_behav['Original_TrialNo'].values - 1) * 6.0
        
        # Build continuous high-resolution neural time courses
        ...
        # Convolve with canonical HRF to obtain hemodynamic behavioral time courses
        ...
        # Aggregate within sliding windows
        ...
        for i in range(n_windows):
            ...
            # Check if this window overlaps with active task trials
        ...
        X_behav_all.append(np.vstack([y_err[valid_windows], y_xf[valid_windows], y_xs[valid_windows]]).T)
...
def main():
    if not os.path.exists(SUBJECT_LIST): return
    with open(SUBJECT_LIST, 'r') as f:
        subjects = [line.strip() for line in f if line.strip()]

    print("\nAligning HRF and compiling dataset (Encoding Model)...")
    ...
    print(f"  > Predictor features X (Behavior): 3 dimensions (e, x_f, x_s)")
    print(f"  > Target Y (Brain): {Y_brain_raw.shape[1]} dimensions (Riemannian space connections)")

    # Create 7 prediction placeholders (in RAW scale)
    Y_pred_full = np.zeros_like(Y_brain_raw)
    Y_pred_no_e = np.zeros_like(Y_brain_raw)
    Y_pred_no_xf = np.zeros_like(Y_brain_raw)
    Y_pred_no_xs = np.zeros_like(Y_brain_raw)
    Y_pred_e = np.zeros_like(Y_brain_raw)
    ...
    print("\nExecuting Leave-One-Subject-Out cross-validation (LOSO) and 7 Ridge modeling sets...")
    fold = 1
    ...
    # Calculate out-of-sample R^2 (truncate negative R^2 to 0 to avoid noise interference in subtraction)
    def calc_r2(pred):
    ...
    r2_no_xf = calc_r2(Y_pred_no_xf)
    r2_no_xs = calc_r2(Y_pred_no_xs)
    r2_e = calc_r2(Y_pred_e)
    r2_xf = calc_r2(Y_pred_xf)
    r2_xs = calc_r2(Y_pred_xs)
    
    # --- Venn Diagram Math ---
    unique_e = np.maximum(0, r2_full - r2_no_e)
    unique_xf = np.maximum(0, r2_full - r2_no_xf)
    unique_xs = np.maximum(0, r2_full - r2_no_xs)
    
    shared_e_xf = np.maximum(0, r2_e + r2_xf - r2_no_xs)
    shared_xf_xs = np.maximum(0, r2_xf + r2_xs - r2_no_e)
    shared_e_xs = np.maximum(0, r2_e + r2_xs - r2_no_xf)
    
    shared_all = np.maximum(0, r2_full - (unique_e + unique_xf + unique_xs + shared_e_xf + shared_xf_xs + shared_e_xs))
    
    print("\nSaving Group-level 7 VPA neural axes (R^2 arrays)...")
    np.save(f"{OUTPUT_DIR}/Unique_e_R2.npy", unique_e)
    np.save(f"{OUTPUT_DIR}/Unique_xf_R2.npy", unique_xf)
    np.save(f"{OUTPUT_DIR}/Unique_xs_R2.npy", unique_xs)
    np.save(f"{OUTPUT_DIR}/Shared_e_xf_R2.npy", shared_e_xf)
    np.save(f"{OUTPUT_DIR}/Shared_xf_xs_R2.npy", shared_xf_xs)
    np.save(f"{OUTPUT_DIR}/Shared_e_xs_R2.npy", shared_e_xs)
    np.save(f"{OUTPUT_DIR}/Shared_all_R2.npy", shared_all)
    ...
    for subj_id in unique_subjs:
        ...
        def calc_r2_sub(pred):
            # Note: Because the individual sample size is small here, R2 variation is large, so negative values are also forcibly truncated to prevent contaminating VPA subtraction
            return np.maximum(0, r2_score(Y_sub, pred[idx], multioutput='raw_values'))
            
        sr2_full = calc_r2_sub(Y_pred_full)
        sr2_no_e = calc_r2_sub(Y_pred_no_e)
        sr2_no_xf = calc_r2_sub(Y_pred_no_xf)
        sr2_no_xs = calc_r2_sub(Y_pred_no_xs)
        sr2_e = calc_r2_sub(Y_pred_e)
        sr2_xf = calc_r2_sub(Y_pred_xf)
        sr2_xs = calc_r2_sub(Y_pred_xs)
        
        sunique_e = np.maximum(0, sr2_full - sr2_no_e)
        sunique_xf = np.maximum(0, sr2_full - sr2_no_xf)
        sunique_xs = np.maximum(0, sr2_full - sr2_no_xs)
        
        sshared_e_xf = np.maximum(0, sr2_e + sr2_xf - sr2_no_xs)
        sshared_xf_xs = np.maximum(0, sr2_xf + sr2_xs - sr2_no_e)
        sshared_e_xs = np.maximum(0, sr2_e + sr2_xs - sr2_no_xf)
        
        sshared_all = np.maximum(0, sr2_full - (sunique_e + sunique_xf + sunique_xs + sshared_e_xf + sshared_xf_xs + sshared_e_xs))
        
        subj_str = f"sub-{int(subj_id):02d}"
        np.save(f"{OUTPUT_DIR}/Unique_e_R2_{subj_str}.npy", sunique_e)
        np.save(f"{OUTPUT_DIR}/Unique_xf_R2_{subj_str}.npy", sunique_xf)
        np.save(f"{OUTPUT_DIR}/Unique_xs_R2_{subj_str}.npy", sunique_xs)
        np.save(f"{OUTPUT_DIR}/Shared_e_xf_R2_{subj_str}.npy", sshared_e_xf)
        np.save(f"{OUTPUT_DIR}/Shared_xf_xs_R2_{subj_str}.npy", sshared_xf_xs)
        np.save(f"{OUTPUT_DIR}/Shared_e_xs_R2_{subj_str}.npy", sshared_e_xs)
        np.save(f"{OUTPUT_DIR}/Shared_all_R2_{subj_str}.npy", sshared_all)
    
    print("All VPA matrices (including group and individual) have been successfully output to the ml_results folder! Ready for subsequent 3D mapping and AFNI statistics.")
