...
|:------|:------|-----:|:----------|
| Cortex | Schaefer 400 | 400 | Data-driven functional parcels aligned to 7-network topology |
| Subcortex + Cerebellum | AAL3v2 | 82 | Anatomically standardized; captures cerebellar lobules IV/V and substantia nigra — both predicted by Smith et al. (2006)'s dual-rate model, absent from cortex-only atlases |
| Seed | Individualized M1 | 1 per subject | M1 hand-area varies ~1–2 cm across individuals; localized per-subject via task-evoked GLM within a precentral gyrus mask, not a group-average coordinate |

**Results** (N=10 pilot, VPA R² thresholded at 0.01):

| Component | Mean R² | Peak R² | Top ROI | ROIs with R² > 0 |
|:----------|--------:|--------:|:--------|:----------------:|
| Unique $x_f$ (fast) | 0.0064 | **0.466** | SomMot_33 (somatomotor cortex) | 27 / 481 (6%) |
| Unique $x_s$ (slow) | 0.0020 | **0.107** | Vis_9 + AAL3_SN_pr_L (substantia nigra) | 19 / 481 (4%) |
| Shared $x_f$–$x_s$ | 0.0054 | **0.200** | SomMot_24 + Cerebellum_4_5_L | 37 / 481 (8%) |
| Limbic negative control | **0.0000** | — | ✅ 4 / 4 Pass | — |
...
|:-------------------:|:------------------------:|
| ![Shared](docs/figures/VPA_Map_Shared_xf_xs_R2_OrthoView.png) | ![Full](docs/figures/VPA_Map_Full_xf_xs_R2_OrthoView.png) |

Results are directionally consistent with Smith et al. (2006): $x_f$ localizes to somatomotor cortex; $x_s$ to visual cortex, somatomotor, and substantia nigra. **No result reaches statistical significance at N=10** — full assessment including permutation tests: [Pilot_Results_Public_v2.md](docs/Pilot_Results_Public_v2.md).
...
| **3A — Riemannian Manifold Projection** | FC matrices are SPD and lie on a curved manifold — not Euclidean space. Each subject's **resting-state covariance is used as their personal reference point** on the SPD manifold; each task-window covariance is projected into the **tangent space at that reference** (Log-Euclidean map), removing inter-individual baseline differences while preserving within-subject dynamics | [`run_hybrid_riemannian_extraction.py`](codes/version_2/run_hybrid_riemannian_extraction.py) |
| **3B — Sliding-Window Dynamic FC** | **Window: 30 TR (60 s) · Step: 1 TR (2 s)** · Ledoit-Wolf shrinkage ensures positive-definiteness in short windows (30 TR << 482 ROIs); extract M1 seed connectivity vector per window; recover 481-ROI label alignment | [`run_hybrid_riemannian_extraction.py`](codes/version_2/run_hybrid_riemannian_extraction.py), [`fix_roi_labels.py`](codes/version_2/fix_roi_labels.py) |
...
- **Phase 2 — Multi-Atlas Node Construction**: Builds the three-layer hybrid parcellation. **Schaefer 400** provides data-driven cortical parcels aligned to the 7-network functional topology. **AAL3v2 (82 ROIs)** adds anatomically standardized subcortical and cerebellar coverage — critical for detecting the cerebellar (lobules IV/V) and substantia nigra contributions predicted by Smith et al. (2006)'s dual-rate model, which are absent from cortex-only atlases. **Individualized M1** (1 ROI per subject, from Phase 1) ensures the seed is anatomically accurate per person rather than averaged across the group. Output: one consistent 482-ROI parcellation with whole-brain coverage.
- **Phase 3A — Riemannian Manifold Projection**: Functional connectivity matrices are Symmetric Positive Definite (SPD) and lie on a curved Riemannian manifold — Euclidean operations applied directly to SPD matrices (subtraction, averaging) can violate positive-definiteness and conflate each subject's idiosyncratic resting-state baseline with genuine task-driven change. Each subject's resting-state covariance is used as their **personal reference point on the SPD manifold**; each task-state covariance is projected into the **tangent space at that reference** via the Log-Euclidean map (Pennec et al. 2006; Varoquaux et al. 2010). This removes inter-individual baseline differences while preserving within-subject task dynamics, and yields a flat Euclidean feature space for all downstream regression. See [Riemannian_Manifold_Projection_Review.md](docs/Riemannian_Manifold_Projection_Review.md) for a full comparison vs. Pearson / partial correlation / ICA / GNN.
- **Phase 3B — Sliding-Window Dynamic FC**: **Window: 30 TR (60 s) · Step: 1 TR (2 s).** The 30-TR window is long enough for stable covariance estimation and short enough to resolve fast adaptation dynamics (adaptation saturates within ~30–60 trials, i.e., ~60–120 s). Stepping by 1 TR gives a dense temporal sampling matched to the behavioral trial rate. Ledoit-Wolf shrinkage regularizes each window's covariance to guarantee positive-definiteness when TR count << ROI count (30 << 482). The output is a time series of tangent-projected M1 seed connectivity vectors — one per TR — capturing how M1 connectivity evolves across the learning trajectory. `fix_roi_labels.py` recovers the correct 481-ROI label alignment for the resulting feature vectors.
